...
(function(){
  ...
  /* ---------- popup de discord ---------- */
  var dcModal = document.getElementById('dc-modal');
  var dcOpen  = document.getElementById('dc-open');
  if (dcOpen && dcModal){
    dcOpen.addEventListener('click', function(){ dcModal.hidden = false; });
    dcModal.addEventListener('click', function(e){
      if (e.target.closest('[data-close]')) dcModal.hidden = true;
    });
    document.addEventListener('keydown', function(e){
      if (e.key === 'Escape') dcModal.hidden = true;
    });
  }

  /* ---------- musica ----------
     El archivo va en audio/theme.mp3
     Si no existe, el reproductor no aparece.
     Empieza al 5% y NO suena sola: los navegadores bloquean
     el audio automatico, hay que darle play.                    */
  var aud  = document.getElementById('m-audio');
  var mBox = document.getElementById('music');
  ...
  if (aud && mBox){
    ...
    /* solo mostramos el reproductor si el archivo existe de verdad */
    fetch(aud.getAttribute('src'), { method: 'HEAD' })
      .then(function(r){ if (r.ok) mBox.hidden = false; })
      .catch(function(){ /* sin archivo: se queda oculto */ });

    mBtn.addEventListener('click', function(){
      if (aud.paused){
        aud.play().then(function(){
          mBtn.textContent = '❚❚'; mBtn.classList.add('on');
        }).catch(function(){});
      } else {
        aud.pause();
        mBtn.textContent = '▶'; mBtn.classList.remove('on');
      }
    });

    mVol.addEventListener('input', function(){
      var v = parseInt(mVol.value, 10);
      aud.volume = v / 100;
      mVal.textContent = v + '%';
      try { localStorage.setItem('wlb-vol', v); } catch(e){}
    });

    var sv = null;
    try { sv = localStorage.getItem('wlb-vol'); } catch(e){}
    if (sv !== null){
      mVol.value = sv;
      aud.volume = parseInt(sv,10) / 100;
      mVal.textContent = sv + '%';
    }
  }
